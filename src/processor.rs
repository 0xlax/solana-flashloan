...
use crate::{instruction::FlashloanProgramInstruction, error::FlashloanError, state::FlashloanProgram};

pub struct Processor;
impl Processor {
    pub fn process(program_id: &Pubkey, accounts: &[AccountInfo], instruction_data: &[u8]) -> ProgramResult {
        let instruction = FlashloanProgramInstruction::unpack(instruction_data)?;
	...
	fn process_init_my_flashloan_program(
		...
		if !rent.is_exempt(flashloan_program_account.lamports(), flashloan_program_account.data_len()) {
			return Err(FlashloanProgramError::NotRentExempt.into());
		}

		let mut flashloan_program_info = FlashloanProgram::unpack_unchecked(&flashloan_program_account.data.borrow())?;
		...
		FlashloanProgram::pack(flashloan_program_info, &mut flashloan_program_account.data.borrow_mut())?;
