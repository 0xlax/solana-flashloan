...
impl Sealed for FlashloanProgram {}

impl IsInitialized for FlashloanProgram {
    fn is_initialized(&self) -> bool {
        self.is_initialized
    }
}

impl Pack for FlashloanProgram {
    const LEN: usize = 65;
    fn unpack_from_slice(src: &[u8]) -> Result<Self, ProgramError> {
        let src = array_ref![src, 0, FlashloanProgram::LEN];
        let (
            is_initialized,
            initializer_pubkey,
	...
	fn pack_into_slice(&self, dst: &mut [u8]) {
        let dst = array_mut_ref![dst, 0, FlashloanProgram::LEN];
        let (
            is_initialized_dst,
